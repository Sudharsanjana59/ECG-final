...
2. **Pick a level** (1 → 16, one level per rhythm — Normal Sinus Rhythm,
   ...
   WPW Syndrome, Sick Sinus Syndrome, LVH, and RVH). Each level unlocks
   after you get a perfect score on the one before it.
...
- **Reset to exactly 16 rhythms/levels**, matching the requested diagnosis
  list one-for-one: Normal Sinus Rhythm, Atrial Flutter, Atrial
  ...
  Syndrome, Sick Sinus Syndrome, LVH, and RVH. Every level is still
  exactly one topic, and no topic repeats across the 16 levels.
- **Every level now has its own color + icon**, generated by spacing 16
...
- **Snappier drag-and-drop**: dropping a piece now pulses it into place
  and the slots keep a clear visible gap between them (`.track` gap +
  a `slotSnap` animation in `css/style.css`), so it's obvious each piece
  is its own puzzle block even once placed.
- **Admin dashboard** with tabs (Overview / Dataset / Levels /
  Submissions), colorful stat cards, an animated bar chart of average
  score per level, and a top-3 podium.
